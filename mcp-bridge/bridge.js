...
const TOOLS = [
  ...
  { name: 'type', description: 'Type text into an input or selector', inputSchema: { type: 'object', properties: { selector: { type: 'string' }, text: { type: 'string' }, frameId: { type: 'number' }, clear: { type: 'boolean', description: 'Clear existing value first' }, perChar: { type: 'boolean', description: 'Type character-by-character with per-char events (for autocomplete/masked inputs)' } }, required: ['selector','text'] } },
...
function toolToAction(name, args) {
  ...
  switch (name) {
    ...
    case 'type': return { action: 'type', selector: args.selector, text: args.text, frameId: args.frameId, clear: args.clear === true, perChar: args.perChar === true };
