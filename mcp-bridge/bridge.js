...
// --- tool definitions ---
const TOOLS = [
  { name: 'tab_navigate', description: 'Navigate the active/target tab to a new URL', readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true, inputSchema: { type: 'object', properties: { url: { type: 'string', description: 'URL to navigate to' } }, required: ['url'] } },
  { name: 'tab_eval', description: 'Evaluate JavaScript in the active tab. Requires explicit allowEval:true because this executes with page privileges.', readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: true, inputSchema: { type: 'object', properties: { code: { type: 'string', description: 'JavaScript expression' }, allowEval: { type: 'boolean', const: true, description: 'Explicit consent for unrestricted page eval' } }, required: ['code', 'allowEval'] } },
  { name: 'tab_list', description: 'List all open tabs in current window with IDs, titles, URLs, and active states', readOnlyHint: true, destructiveHint: false, idempotentHint: true, openWorldHint: false, inputSchema: { type: 'object', properties: {} } },
  { name: 'tab_switch', description: 'Switch agent target tab by tab ID (marks it with a green [👻 Specter] tab group so you know what it is working on)', readOnlyHint: false, destructiveHint: false, idempotentHint: true, openWorldHint: false, inputSchema: { type: 'object', properties: { tabId: { type: 'number', description: 'ID of the tab to target' }, focusWindow: { type: 'boolean', description: 'Bring to foreground (default true, set false for silent background work)' } }, required: ['tabId'] } },
  { name: 'tab_new', description: 'Open a new tab with an optional URL (automatically joins the Specter group)', readOnlyHint: false, destructiveHint: false, idempotentHint: false, openWorldHint: true, inputSchema: { type: 'object', properties: { url: { type: 'string', description: 'Optional initial URL' }, active: { type: 'boolean', description: 'Focus the tab immediately (default true)' } } } },
  { name: 'tab_close', description: 'Close a tab by ID (or target tab if omitted)', readOnlyHint: false, destructiveHint: true, idempotentHint: false, openWorldHint: false, inputSchema: { type: 'object', properties: { tabId: { type: 'number', description: 'Optional tab ID to close' } } } },
